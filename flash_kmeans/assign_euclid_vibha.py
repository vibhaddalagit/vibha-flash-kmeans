...
# 
# FA2-style optimization attempted:
#   - Fused min/argmin reduction: Use only argmin and extract min value
#     via one-hot masking, reducing from 2 reductions to 1.
#   - This mirrors FA2's approach of reducing non-matmul FLOPs.
#   - Result: Minimal impact since k-means lacks softmax complexity.
#
# Inputs:
...
def _euclid_assign_kernel(
    ...
    for k_start in range(0, K, BLOCK_K):
        ...
        # ------------------------------------------------------------------
        # FA2-style fused reduction: single argmin + extract min value
        # Instead of: curr_min = tl.min(dist, axis=1)  # reduction 1
        #             curr_idx = tl.argmin(dist, axis=1)  # reduction 2
        # We use only argmin and extract the min value via one-hot masking
        # ------------------------------------------------------------------
        curr_idx = tl.argmin(dist, axis=1)  # single reduction
        
        # Extract min values using one-hot mask
        idx_onehot = tl.arange(0, BLOCK_K)[None, :] == curr_idx[:, None]
        curr_min = tl.sum(tl.where(idx_onehot, dist, 0.0), axis=1)
...
def _cosine_assign_kernel(
    ...
    for k_start in range(0, K, BLOCK_K):
        ...
        # Compute cosine similarity (BLOCK_N, BLOCK_K) = x_tile @ c_tile
        cross = tl.dot(x_tile, c_tile).to(tl.float32)  # float32
        ...
        # ------------------------------------------------------------------
        # FA2-style fused reduction: single argmax + extract max value
        # ------------------------------------------------------------------
        curr_idx = tl.argmax(sim, axis=1)  # single reduction
        
        # Extract max values using one-hot mask
        idx_onehot = tl.arange(0, BLOCK_K)[None, :] == curr_idx[:, None]
        curr_max = tl.sum(tl.where(idx_onehot, sim, 0.0), axis=1)
